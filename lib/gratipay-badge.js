...
page.open(__dirname + '/index.html', function (status) {
  ...
  waitFor(function waitForImageToLoad () {
    ...
    var imageSvg = getImageStr();
    var destPath = __dirname + '/../dist/gratipay.svg';
    fs.write(destPath, imageSvg, 'w');
    console.log('Wrote gratipay badge to "' + destPath + '"');
    phantom.exit(0);
  });
  return;
  ...
  waitFor(function waitForImageToLoad () {
    ...
    var destPath = __dirname + '/../dist/gratipay.png';
